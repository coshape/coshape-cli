...
exports.createWorkspace = function( settings /*workspace_path, build_path, template_path, project_path*/) {
	...
	_set.workspace.build_path = utils.normalizePath(_set.workspace.build_path);
	_set.workspace.templates_path = utils.normalizePath(_set.workspace.templates_path);
	_set.workspace.projects_path = utils.normalizePath(_set.workspace.projects_path);
	_set.workspace.settings_path = utils.normalizePath(_set.workspace.settings_path);
	...
	if (fs.existsSync(_set.workspace.settings_path))  throw("settigns file already exists, abort.");
  	...
  	utils.mkdirp(_set.workspace.workspace_path, (err) => {
  		if (err) {throw(err);}
  			...
  			var base_path = _set.workspace.workspace_path;
  			var set_abs_path = _set.workspace.settings_path;
  			_set.workspace.build_path = utils.pathRelative(base_path, _set.workspace.build_path);
  			_set.workspace.templates_path = utils.pathRelative(base_path, _set.workspace.templates_path);
  			_set.workspace.projects_path = utils.pathRelative(base_path, _set.workspace.projects_path);
  			_set.workspace.settings_path = utils.pathRelative(base_path, _set.workspace.settings_path);

  			// store settings file ...
  			utils.storeJsonFile(set_abs_path, _set);
			utils.info(_set);
  		}
  	});
}
