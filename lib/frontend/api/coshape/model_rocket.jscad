...
// Here we define the user editable parameters: 
  ...
  function getParameterDefinitions() {
    return [
      ...
      { name: 'tube_dia', caption: 'tube diameter', type: 'float', default: 18 },
...
function cone(dia, height) {
  ...
  radius: rho,
  startangle: 0,
  endangle: asin(height/rho)*180/acos(-1),
  resolution: 256
});
...
function main(p)
{
  ...
  group.push(fins(p.bladeN,p.fin_height, p.fin_length, 15, p.tube_thickness, p.tube_dia/2, -p.fin_unit_height).translate([0,0,-p.tube_height/3*2]));
    ...
    group.push(cone(p.tube_dia, p.cone_height).translate([0,0,p.tube_height]).translate([0,0,-p.tube_height/3*2]));
