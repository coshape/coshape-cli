...
function getParameterDefinitions() {
  return [
   { name: 'length', type: 'float', default: 100, caption: "Length:" },
    ...
    { name: 'height', type: 'float', default: 5, caption: "Height:" },
    //{ name: 'depth', type: 'float', default: 70, caption: "Depth:" },
    { name: 'cut_out', type: 'choice', caption: 'Cut Out', 
    values: [0, 1, 2], 
    captions: ["No Cut Out", "Display Cut out", "Honeycomb"], default: 2 },
    { name: 'cut_x', type: 'float', default: 15, caption: "Left:" },
    { name: 'cut_y', type: 'float', default: 10, caption: "Top:" },
    { name: 'cut_width', type: 'float', default: 40, caption: "Length:" },
    { name: 'cut_height', type: 'float', default: 40, caption: "Width:" },
    { name: 'feature_size', type: 'float', default: 10, caption: "Pattern Size:" }
...
/*
 ...
 // var bottom = difference(outer, inner);
  ...
  else if(params.cut_out == 2)
  {

    var cr = params.feature_size;
    var comb = cylinder({r:cr, h:d*2, fn:6});
    

    var x=0;var y=0;
    var cdy = 0.8660254037844386 * cr+cr/4;
    var cdx = 3*cr+ 0.8660254037844386*cr;
    var sx = params.cut_width / cdx;
    var sy = params.cut_height / cdy;
    for(var ix=0; ix<sx; ix++) {
      for(var iy=0; iy<sy; iy++) {
        x = -params.cut_width/2+params.cut_x+ix*cdx-cr;
        if( iy % 2 == 0)
          x += cdx/2;
        y = -params.cut_height/2+params.cut_y+iy*cdy;
        if(x>-params.cut_width/2+params.cut_x)
        if(y>-params.cut_height/2+params.cut_y)
        if(x<params.cut_width/2+params.cut_x)
        if(y<params.cut_height/2+params.cut_y)
        top = difference( top, comb.translate([x,y,0]));
      }
    }
  }
