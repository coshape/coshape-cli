...
XMOD.Customizer = new function() {
	...
	this.append_shape = function(
		{
			type="jscad", name, parent_name, data, transform_callback, io, material, parameters_callback, use_b64=false, clone_function=null
		}) {
		...
		if (!!type) {
			...
			let shape = { name: name, data:data, type: type, use_b64:use_b64}
			console.log("append_shape", type, name, parent_name)

			let parent_node = null
			if (parent_name) {
				let parentid = XMOD.Customizer.componentIdsByName[parent_name]
				if (parentid==parentid) {
					parent_node = XMOD.Customizer.components[parentid]
				}
			}

			generate_shape(shape, parent_node, function(item) {
				item.name = shape.name
				item._shape_type = shape.type
				item.transform_callback = transform_callback.bind(item);
				item.parameters_callback = parameters_callback.bind(item);
				...
				XMOD.Customizer.componentIdsByName[item.name] = XMOD.Customizer.components.length -1
	...
	this.update_component_trans = function(item, io) {
		console.log("update component", item.name)
		if (item){
	...
	this.update_component_shape = function(item, io, modifications) {
		if (item){
			let _io = Object.assign({}, io);
			for (var [key, value] of Object.entries(_io)) {
		  	_io[key] = _io[key]*1
			}
			//console.log("IO before", JSON.stringify(io))
			var paras = item.parameters_callback(_io, item);

			// io = Object.assign({}, _io);

			if (typeof item._local_io !== 'undefined') {
				//console.log("my io");
				//console.log(item._local_io);
				let loc_io = Object.assign({}, item._local_io);
				paras = item.parameters_callback(loc_io, item);
			}
			...
			const component_clone_transform = function(item) {
				...
				//_io = Object.assign({}, io);
				var t = item.transform_callback(_io, true);
				...
				console.log("TRS update", item.name);
				...
				//console.log("IO after", JSON.stringify(_io))
	...
	function update() {
		if (is_initialized && shape_code) {
			generate_shape(shape_code, item._parent_node, function(item) {
	...
	function generate_shape(shape, parent_node, callback) {

		if (shape && shape.type && XMOD.modeler) {
			if (shape.type == "jscad") {
				XMOD.modeler.createBody(shape.data, parent_node, callback)
