...
XMOD.CModeler = function() {
  ...
  this.duplicateSelected = function() {
    if (this.SELECTED) {
      this.createBody(this.SELECTED.code, this.SELECTED._parent_node, function(mesh) {
  ...
  this.createBody = function(code, parent_node, callback = function(mesh) {}) {
    var mesh = this.createMeshNode();
    ...
    mesh.setCode(code, function() {
      ...
      if (parent) {
        parent.add(mesh)
        mesh._parent_node = parent
      } else {
        thisobj.part_group.add(mesh)
        mesh._parent_node = null
      }
