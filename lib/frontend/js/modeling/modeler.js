...
XMOD.CModeler = function() {
  ...
  this.create_workspace = function(dims) {
    ...
    var GridHelperA = function(width, depth, step, color1, color2) {
      ...
      var vertices = [],
        colors = [];
      half_dimx = width / 2; //dims[0]/2;
      half_dimz = depth / 2; //dims[1]/2;
      var ws = width / step;
      var ds = depth / step;
      var w_center = ws / 2;
      var d_center = ds / 2;
      ...
      var k = 0;
      var j = 0;
      for (var i = 0, k = -half_dimx; i <= ws; i++, k += step) {
        vertices.push(k, 0, -half_dimz, k, 0, half_dimz);
      ...
      for (var i = 0, k = -half_dimz; i <= ds; i++, k += step) {
        vertices.push(-half_dimx, 0, k, half_dimx, 0, k);
    ...
    const dim_scale = 50 // org was 50
    // add new items
    var gridHelper = new GridHelperA(dims[0], dims[2]);
    var ticks = (dims[0]) / 500;
    for (var ti = 0; ti <= ticks; ti++) {
      var tn = this.create_text_node(ti * dim_scale + "cm", 20);
      if (tn) {
    ...
    var label = this.create_text_node("WIDTH", 50);
    if (label) {
      label.position.z = dims[2] / 2 + 120;
      label.position.x = 0;
    ...
    gridHelper.position.y = -dims[1] / 2
    ...
    label = this.create_text_node("DEPTH", 50);
    if (label) {
      label.position.x = dims[0] / 2 + 120;
      label.position.z = 0
      label.rotation.x = -Math.PI / 2.0;
    ...
    if (!this._ws.plane1) {
      var geometry = new THREE.PlaneBufferGeometry(dims[0], dims[2]);
      ...
      this.plane = this._ws.plane1 || new THREE.Mesh(geometry, floorMaterial);
      this._ws.plane1 = this.plane;
      // this.objects.push(this.plane);
      geometry = new THREE.PlaneBufferGeometry(dims[0], dims[1]);
      this.plane2 = new THREE.Mesh(geometry, floorMaterial);
      this.plane2.receiveShadow = true;
      this.plane2.position.z = -1000;
      this.plane2.position.y = 1000;
      geometry = new THREE.PlaneBufferGeometry(dims[2], dims[1]);
      geometry.rotateY(Math.PI / 2);
      this.plane3 = new THREE.Mesh(geometry, floorMaterial);
      this.plane3.receiveShadow = true;
      this.plane3.position.x = -1000;
      this.plane3.position.y = 1000;
    } else {
      this._ws.plane1.geometry = new THREE.PlaneBufferGeometry(dims[0], dims[2]);
      this._ws.plane1.geometry.rotateX(-Math.PI / 2);
    }
  }
